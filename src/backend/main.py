...
async def lifespan(app: FastAPI):
    ...
    """
    global chatbot
    logger.info("🚀 Starting up... Initializing chatbot with MCP tools")
    try:
        chatbot = await ChatbotManager.create(enable_mcp=True)
        logger.info(f"✓ Chatbot initialized with {len(chatbot.tools)} tools")
    except Exception as e:
        logger.error(f"⚠️  Failed to initialize MCP tools, falling back to local tools: {e}")
        chatbot = ChatbotManager()  # Fallback to local tools
    
    yield
    
    logger.info("🛑 Shutting down... Cleaning up chatbot")
    if chatbot:
        await chatbot.aclose()
    logger.info("✓ Chatbot cleaned up")
...
async def upload_pdf(thread_id: str, file: UploadFile = File(...)):
    ...
    try:
        await chatbot.add_document_to_thread(thread_id, file)
        return {
            "thread_id": thread_id,
...
if __name__ == "__main__":
    ...
    logger.info("Starting LangGraph Chatbot API with MCP Tools")
    logger.info("📚 Available tools include:")
    logger.info("  - Local: duckduckgo_search, stock_price, rag_tool")
    logger.info("  - MCP: calculator, add_expense, list_expenses, summarize")
    logger.info("")
